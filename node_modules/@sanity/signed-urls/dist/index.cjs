...
function urlWithSigningParams(url, query, signingOptions) {
  ...
  return query.length > 0 && (parts.push(query), parts.push("&")), parts.push(`keyid=${rfc3986(signingOptions.keyId)}`), signingOptions.expiry && parts.push(`&expiry=${rfc3986(normalizeExpiry(signingOptions.expiry) ?? "")}`), parts.join("");
}
